...
function generateCacheKey(url, options) {
	...
	const cacheable =
		...
		fileName +
		type +
		getQueryForCacheKey(parsedUrl, options.useQueryParamsInCacheKey)
...
function downloadImage(fromUrl, toFile, headers = {}) {
	...
	if (!_.has(activeDownloads, toFile)) {
		...
		activeDownloads[toFile] = RNFetchBlob.config({ path: tmpFile })
			...
			.then(res => {
				...
				if (status !== 2) {
					throw {
						message:
							res.respInfo.statusText ||
							'Failed to successfully download image',
						code: res.respInfo.status
					}
				}
...
function runPrefetchTask(prefetcher, options) {
	...
	// else check cache
	return (
		getCachedImagePath(url, options)
...
function isExpired(filePath) {
	return MemoryCache.isExpired(filePath).then(isExpired => {
		if (isExpired === true) {
...
function setFileInfo(url, headers) {
	let maxAge = /^max-age/.test(headers[HEADERS_CACHE_CONTROL])
		? +headers[HEADERS_CACHE_CONTROL].split('=')[1]
		: null
...
function cacheImage(url, options = defaultOptions) {
	...
	return ensurePath(dirPath).then(() =>
		downloadImage(url, filePath, options.source && options.source.headers)
	)
}
...
function deleteMultipleCachedImages(urls, options = defaultOptions) {
	return _.reduce(
		urls,
		(p, url) => p.then(() => deleteCachedImage(url, options)),
		Promise.resolve()
	)
}
...
function seedCache(local, url, options = defaultOptions) {
	const filePath = getCachedImageFilePath(url, options)
	const dirPath = getDirPath(filePath)
	return ensurePath(dirPath).then(() => fs.cp(local, filePath))
}
...
function clearCache(options = defaultOptions) {
	return fs
		.unlink(getBaseDir(options.cacheLocation))
		.catch(() => {
